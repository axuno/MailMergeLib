...
namespace MailMergeLib.SmartFormatMail
{
    ...
    /// </summary>
    /// <remarks>
    /// In case more than one object has the same member (or key) name, the value of the first object in the list will prevail.
    ...
    public class SmartObjects : List<object>
    {
        ...
        public new void Add(object obj)
        {
            if (obj == null)
            {
                throw new ArgumentNullException($"{nameof(obj)} must not be null.", nameof(obj));
            }
        ...
        public new void AddRange(IEnumerable<object> objList)
        {
            ...
            {
                throw new ArgumentException($"Objects of type '{nameof(SmartObjects)}' cannot be nested. At least one object in the argument list has type '{nameof(SmartObjects)}'.", nameof(objList));
            }
